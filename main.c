...
void carregar_programa(void) {
    /* Cada linha representa opcode, operando 1 e operando 2. */
    const uint8_t programa[] = {
        0x01, 0x01, 0x10, /* 0x40: LOAD R1, 0x10 */
        0x05, 0x00, 0x01, /* 0x43: MOV R0, 1 */
        0x05, 0x03, 0x00, /* 0x46: MOV R3, 0 */
        0x06, 0x01, 0x03, /* 0x49: CMP R1, R3 */
        0x08, 0x7F, 0x00, /* 0x4C: JZ 0x7F (0! = 1) */
        0x02, 0x00, 0x21, /* 0x4F: STORE R0, 0x21 */
        0x05, 0x00, 0x00, /* 0x52: MOV R0, 0 */
        0x02, 0x01, 0x22, /* 0x55: STORE R1, 0x22 */
        0x01, 0x02, 0x21, /* 0x58: LOAD R2, 0x21 */
        0x03, 0x00, 0x02, /* 0x5B: ADD R0, R2 */
        0x05, 0x03, 0x01, /* 0x5E: MOV R3, 1 */
        0x04, 0x01, 0x03, /* 0x61: SUB R1, R3 */
        0x05, 0x03, 0x00, /* 0x64: MOV R3, 0 */
        0x06, 0x01, 0x03, /* 0x67: CMP R1, R3 */
        0x09, 0x58, 0x00, /* 0x6A: JNZ 0x58 */
        0x01, 0x01, 0x22, /* 0x6D: LOAD R1, 0x22 */
        0x05, 0x03, 0x01, /* 0x70: MOV R3, 1 */
        0x04, 0x01, 0x03, /* 0x73: SUB R1, R3 */
        0x05, 0x03, 0x00, /* 0x76: MOV R3, 0 */
        0x06, 0x01, 0x03, /* 0x79: CMP R1, R3 */
        0x09, 0x4F, 0x00, /* 0x7C: JNZ 0x4F */
        0x02, 0x00, 0x20, /* 0x7F: STORE R0, 0x20 */
        0x0A, 0x00, 0x00  /* 0x82: HALT */
    };

    /* O salto evita que as instrucoes ocupem os enderecos dos dados. */
    mem[0] = 0x07;
    mem[1] = 0x40;
    mem[2] = 0x00;
    mem[0x10] = N;
...
void decode_execute(uint8_t op, uint8_t a, uint8_t b) {
    ...
    /* O switch decodifica o opcode; cada case executa a operacao. */
    switch (op) {
